...
- `eval/cases/cases.json` — 16 synthetic scan findings with known-correct
  labels, used as ground truth for scoring. Deliberately includes edge
  cases (a knowledge-base miss, an unparseable version, a genuine
  duplicate, a boundary version) and one adversarial case designed to try
  to break the verification logic.
