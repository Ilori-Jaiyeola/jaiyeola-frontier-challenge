...
9. **If chat quota runs out mid-task**, use `docs/continuity_prompt.md` to
   ...
   file for the exact handoff/return prompts. The handoff prompt requires
   the receiving model to produce both a HANDBACK SNAPSHOT (for resuming
   work) and a TRAJECTORY NOTE (for submission disclosure, saved as
   `trajectories/<model>-handoff-<phase>.md`) before ending its session —
   so a return to Claude, or a further handoff to a third model, never
   loses context or disclosure evidence.
10. **Every time a new file or meaningful change lands, update two things
    alongside it**: a trajectory note for that phase in `trajectories/`,
    and `docs/continuity_prompt.md`'s example state so a handoff prompt
    built from it always reflects the actual current repo, not a stale
    snapshot.
