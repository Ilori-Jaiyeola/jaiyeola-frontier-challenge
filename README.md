...
|                                                       |
| 1. MEMORY                                             |
|    Seen this host+plugin before this session?         |
|    -> reuse the prior result                          |
|                                                       |
| 2. TOOL / CONTEXT                                     |
|    Look up plugin_name in the local vulnerability     |
|    knowledge base (eval/knowledge_base/vuln_db.json)  |
|    -> affected version range, vuln class,             |
|       known false-positive notes                      |
|                                                       |
| 3. VERIFICATION (branches by vulnerability class)     |
|    - config-class (version-independent): requires     |
|      DIRECT exploitation evidence to confirm (a real  |
|      login, a real unauthenticated command executing) |
|    - version-dependent: confirm on version match      |
|      UNLESS evidence contradicts exploitability (e.g. |
|      a secure protocol negotiated by default) -- or   |
|      contains an explicit override signal (a          |
|      demonstrated successful exploit)                 |
|                                                       |
| 4. OUTPUT                                             |
|    Label + plain-English reason tied to the specific  |
|    evidence used                                      |
+-------------------------------------------------------+
        |
        v
{label: Confirmed | Likely False Positive | Needs Verification,
 reason: "..."}
```
...
(`eval/knowledge_base/vuln_db.json`), the 16 synthetic test cases with
known-correct labels (`eval/cases/cases.json` — expanded from an initial
10 specifically to include edge cases and one adversarial case, see
Section 5), the naive baseline (`baseline/solution.py`), the agentic
advanced solution (`advanced/solution.py`), and every entry in
`CHANGELOG.md` — all written after reading this problem PDF.
...
See [`trajectories/`](./trajectories/) — four phases captured so far:
`claude-chat-00-setup-and-planning.md` (pre-kickoff scaffold),
`claude-chat-01-triage-solution-build.md` (problem choice through first
working solution), `claude-chat-02-eval-expansion.md` (expanding the eval
set and the adversarial-case discovery), and
`claude-chat-03-video-script.md` (video script and recording setup).
